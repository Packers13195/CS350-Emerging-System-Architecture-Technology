...
class TemperatureMachine(StateMachine):
    ...
    ##
    def run(self):
        self.myThread = Thread(target=self.manageMyDisplay)
        self.myThread.start()
    ...
    def getFahrenheit(self):
        ...
        except OSError as e: #Handle occational temperatur sensor error that was crashing the script

            if DEBUG:

                print(f"I2C Communication Error: {e}")

            return 0 # Return 0 so script doesn't crash. 0 value is placeholder
    
    ##
    ##  Configure output string for the Thermostat Server
    ##
    def setupSerialOutput(self):
        output = (f"{self.current_state.id}, {self.getFahrenheit():.0f}, {self.setPoint}")
        return output
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                lcd_line_2 = f"T:{self.getFahrenheit():.0f}"
                altCounter = altCounter + 1
            else:
                lcd_line_2 = f"S:{self.current_state.id}  SP:{self.setPoint}"
                altCounter = altCounter + 1
                if(altCounter >= 11):
...
##
greyButton = Button(12)
greyButton.when_pressed = tsm.processTempIncButton
